...
function __fetch_secrets --on-event fish_prompt --description "Auto-fetch secrets from 1Password"
    ...
    # Run secrets script silently
    if ~/dotfiles/scripts/secrets.sh > /dev/null 2>&1
        set_color green
        echo "✓"
        set_color normal

        # Re-source secrets file to load newly fetched values
...
function config --description "Manage dotfiles"
    switch $argv[1]
        ...
        case status
            ...
            cd $prev_dir
        case '*'
            echo "Usage: config [edit|sync|reload|status]"
